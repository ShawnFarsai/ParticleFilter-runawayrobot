...
def estimate_next_pos(measurement, OTHER=None):
    # instantiate OTHER or add data to it
    if not OTHER:
        OTHER = []

    OTHER.append(measurement)

    # Need at least 3 readings before we can compute a prediction
    if len(OTHER) > 2:
        distances = []
        headings = []
        turns = []
        ...
        for i in range(1, len(OTHER)):
            ...
            if (slope < 0) & (p1[1] > p2[1]):
                #quadrant 1
                heading = pi + heading
            elif (slope > 0) & (p1[1] < p2[1]):
                #quadrant 2
                heading = pi + abs(heading)
            elif (slope < 0) & (p1[1] < p2[1]):
                #quadrant 3
        ...
        average_turn = sum(turns) / len(turns)

        predicted_heading = angle_trunc(headings[-1] + average_turn)

        x_current = OTHER[-1][0]
        y_current = OTHER[-1][1]

        predicted_x = x_current + average_distance * cos(predicted_heading)
        predicted_y = y_current + average_distance * sin(predicted_heading)
        
        xy_estimate = (predicted_x, predicted_y)
    else:
